...
export default function RestaurantSignupEmail() {
  ...
  // Handle redirect results from Firebase Google Sign-in
  useEffect(() => {
      ...
      try {
        ...
        unsubscribe = onAuthStateChanged(firebaseAuth, async (user) => {
          if (user && !redirectHandledRef.current) {
            console.log("🔔 [Auth] Restaurant Signup state changed:", user.email)
        ...
        try {
          ...
          if (result?.user && !redirectHandledRef.current) {
            console.log("✅ [Auth] Redirect result found in Restaurant Signup")
  ...
  const processSignedInUser = useCallback(async (user, source = "unknown") => {
    ...
    try {
      ...
      console.error(`Error processing restaurant from ${source}:`, error)
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      try {
        ...
        if (result?.user) await processSignedInUser(result.user, "popup")
      } catch (e) {
            ...
            <CardContent className="space-y-5">
              ...
              {step === 1 && (
                ...
                <Button
                  ...
                  className="w-full h-11 border-gray-300 hover:bg-gray-50 flex items-center justify-center gap-3 font-semibold text-gray-700 active:scale-[0.98]"
                  disabled={isLoading}
                >
