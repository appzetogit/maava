...
export default function LandingPageManagement() {
  ...
  // Settings
  const [settings, setSettings] = useState({ exploreMoreHeading: "More on Maava" })
  ...
  const fetchSettings = async () => {
    try {
      ...
      if (response.data.success) {
        setSettings(response.data.data.settings || { exploreMoreHeading: "More on Maava" })
      }
    } catch (err) {
      // Silently handle 401/404 errors - endpoints may not exist yet, use default settings
      if (err.response?.status === 401 || err.response?.status === 404) {
        setSettings({ exploreMoreHeading: "More on Maava" }) // Use default settings
        setError(null) // Clear any previous error
      } else {
        // Filter out token-related errors
