...
export default function SupportPage() {
  ...
  const fetchSupportData = async () => {
    try {
      ...
      toast.error('Failed to load support page')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const htmlContent = supportData.content.split('\n').map(line => {
        if (line.trim() === '') return '<p><br></p>'
        return `<p>${line}</p>`
      }).join('')
      
      const response = await api.put(API_ENDPOINTS.ADMIN.SUPPORT, {
      ...
      toast.error(error.response?.data?.message || 'Failed to save support page')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="p-4 lg:p-6 bg-slate-50 min-h-screen">
      <div className="max-w-6xl mx-auto">
        {/* Page Header */}
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-slate-900">Support & FAQ</h1>
          <p className="text-sm text-slate-600 mt-1">Manage your Support and Frequently Asked Questions</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md border border-slate-200 p-6">
          <Textarea
            ...
            placeholder="Enter support content, FAQs, and contact information..."
