...
export default function AdminSidebar({ isOpen = false, onClose, onCollapseChange }) {
  ...
  // Filter menu items based on search query and domain
  const filteredMenuData = useMemo(() => {
    const isHibermartMatch = location.pathname.includes('/hibermart');

    // Choose the base menu data based on active module
    const currentDomainMenu = isHibermartMatch ? hibermartSidebarMenuData : sidebarMenuData;

    if (!searchQuery.trim()) {
      return currentDomainMenu
    }

    const query = searchQuery.toLowerCase().trim()
    const filtered = []

    currentDomainMenu.forEach((item) => {
      if (item.type === "link") {
  ...
  return (
    <>
      ...
      >
        ...
        <div className={cn(
          "px-3 py-3 border-b border-neutral-800/60 animate-[fadeIn_0.4s_ease-out]",
          location.pathname.includes('/hibermart') ? "bg-black" : "bg-neutral-900"
        )}>
            ...
            {!isCollapsed && (
              <div className="flex items-center gap-2 animate-[slideIn_0.3s_ease-out]">
                <div className="w-24 h-12 rounded-lg flex items-center justify-center shadow-black/20">
                  {location.pathname.includes('/hibermart') ? (
                    <div className="flex flex-col items-center">
                      <span className="text-white font-[1000] text-lg tracking-tighter italic leading-none">HIBERMART</span>
                      <span className="text-white/40 font-black text-[7px] uppercase tracking-[0.3em] mt-1">Global Admin</span>
                    </div>
                  ) : (
                    logoUrl ? (
                      <img
          ...
          {!isCollapsed && (
            <div className="mb-3 animate-[slideIn_0.4s_ease-out_0.1s_both]">
              <h2 className="text-sm font-semibold text-neutral-300 uppercase tracking-wider text-left">
                {location.pathname.includes('/hibermart') ? "Hibermart Admin" : "Admin Panel"}
              </h2>
            </div>
          )}
