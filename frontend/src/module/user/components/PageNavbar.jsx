...
export default function PageNavbar({
  ...
  useEffect(() => {
    if (location &&
      ...
      debugLog("🔄 Auto-triggering location fetch due to placeholder values")
  ...
  const locationDisplay = (() => {
    ...
    if (!mainLocation &&
      ...
      if (filteredParts.length === 0 ||
        ...
        debugLog("⚠️ Address field only contains placeholder, skipping")
    ...
    // Skip if city is "Current Location" or "Select location" - these are placeholders
      ...
      debugLog("⚠️⚠️⚠️ FALLBACK: Using city (no locality found):", mainLocation)
    }
    // Final fallback: Show "Select location" instead of coordinates
    else if (!mainLocation) {
      mainLocation = "Select location"
      debugLog("⚠️ No valid location found, showing placeholder")
    }
    ...
    if (isCoordinates(mainLocation)) {
      ...
      debugLog("⚠️ Replaced coordinates with:", mainLocation)
    }
    ...
    if (hasCity && hasState) {
        ...
        if (parts.length >= 4) {
          ...
          if (!subLocation && parts.length >= 6) {
            ...
            if (cityPart && statePart &&
              ...
              debugLog("⚠️ Validation failed for parts[4] and parts[5]:", { cityPart, statePart })
            }
          }
        }
      }
      ...
      if (!subLocation) {
        subLocation = ""
        debugLog("⚠️ Could not extract city/state from address")
      }
    }
  ...
  return (
    <nav
      className={`relative pt-0 sm:pt-0 pb-1 sm:pb-2 lg:pb-4 ${zIndexClass} w-full px-1 pr-2 sm:px-2 sm:pr-3 md:px-3 lg:px-6 xl:px-8`}
      onClick={onNavClick}
    >
          ...
          <Link to="/user" className="flex items-center justify-center">
            {logoUrl ? (
              ...
              <div className="flex flex-col items-center">
                <span className={`${textColorClass} font-[1000] text-2xl tracking-tighter italic leading-none drop-shadow-lg`}>MAAVA</span>
                <span className={`${textColor === "white" ? "text-white/60" : "text-black/40"} font-black text-[8px] uppercase tracking-[0.3em] mt-1`}>Food & More</span>
              </div>
            )}
          </Link>
        )}
