...
export default function RestaurantDetails() {
      ...
      try {
        ...
        if (apiRestaurant) {
          ...
          const formatRestaurantAddress = (locationObj) => {
            ...
            const addressParts = []
            if (locationObj.addressLine1 && locationObj.addressLine1.trim() !== "") addressParts.push(locationObj.addressLine1.trim())
            if (locationObj.addressLine2 && locationObj.addressLine2.trim() !== "") addressParts.push(locationObj.addressLine2.trim())
            if (locationObj.area && locationObj.area.trim() !== "") addressParts.push(locationObj.area.trim())
            if (locationObj.city && locationObj.city.trim() !== "") addressParts.push(locationObj.city.trim())
            if (locationObj.state && locationObj.state.trim() !== "") addressParts.push(locationObj.state.trim())

            const pinCode = locationObj.pincode || locationObj.zipCode || locationObj.postalCode
            if (pinCode && pinCode.toString().trim() !== "") addressParts.push(pinCode.toString().trim())

            if (addressParts.length >= 1) return addressParts.join(', ')
          ...
          const transformedRestaurant = {
            ...
            cuisine: (actualRestaurant?.cuisines && Array.isArray(actualRestaurant.cuisines) && actualRestaurant.cuisines.length > 0)
              ...
              : (actualRestaurant?.cuisine || apiRestaurant?.cuisine || "Multi-cuisine"),
            rating: actualRestaurant?.rating ?? apiRestaurant?.rating ?? null,
            reviews: actualRestaurant?.totalRatings ?? apiRestaurant?.totalRatings ?? 0,
            deliveryTime: actualRestaurant?.estimatedDeliveryTime || apiRestaurant?.estimatedDeliveryTime || null,
            distance: calculatedDistance || actualRestaurant?.distance || apiRestaurant?.distance || null,
            location: formattedAddress,
            locationObject: locationObj,
            image: actualRestaurant?.profileImage?.url || apiRestaurant?.profileImage?.url || "https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?w=800&h=600&fit=crop",
            priceRange: actualRestaurant?.priceRange || apiRestaurant?.priceRange || null,
            ...
            slug: actualRestaurant?.slug || apiRestaurant?.slug || slug || "unknown",
            restaurantId: actualRestaurant?.restaurantId || actualRestaurant?._id || null
          }
          ...
          if (!restaurantIdForMenu && zoneId) {
            try {
              const searchParams = { limit: 100, zoneId: zoneId }
              const searchResponse = await restaurantAPI.getRestaurants(searchParams)
              const restaurants = searchResponse?.data?.data?.restaurants || []
              const matchingRestaurant = restaurants.find(r =>
                r.name?.toLowerCase().trim() === transformedRestaurant.name?.toLowerCase().trim()
              )
              if (matchingRestaurant) {
                restaurantIdForMenu = matchingRestaurant._id || matchingRestaurant.restaurantId
                setRestaurant(prev => ({ ...prev, id: restaurantIdForMenu, restaurantId: restaurantIdForMenu }))
              }
            } catch (searchError) { /* ignore */ }
          }
  ...
  return (
    ...
    >
      ...
      <div className="px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12 pt-3 md:pt-4 lg:pt-5 pb-2 md:pb-3 bg-white dark:bg-[#1a1a1a] relative z-30">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-3 mb-4">
            ...
            {/* Search Bar - Moved from below filters */}
            <div className="flex-1 max-w-[220px] relative group mx-auto">
              <div className="absolute inset-y-0 left-2.5 flex items-center pointer-events-none">
                <Search className="h-3.5 w-3.5 text-gray-400 group-focus-within:text-red-500 transition-colors" />
              </div>
              <input
                type="text"
                placeholder="Search"
                ...
                className="w-full h-8 bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-full pl-8 pr-8 text-[11px] font-bold placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all font-outfit"
              />
              {searchQuery && (
                <button
      ...
      {/* Main Content Card - Re-architected for premium look */}
      <div className="bg-white dark:bg-[#0a0a0a] rounded-t-3xl relative z-20 -mt-3">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12 py-4">

          <div className="flex justify-between items-start mb-3">
            <div className="space-y-1.5 min-w-0 flex-1">
              <h1 className="text-[22px] sm:text-[28px] font-black tracking-tightest leading-[1.1] mb-1 font-outfit">
                <span className="bg-clip-text text-transparent bg-gradient-to-b from-gray-950 to-gray-700 dark:from-white dark:to-gray-400">
      ...
      <div className="bg-white dark:bg-[#0a0a0a] px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12 relative z-20">
        <div className="max-w-7xl mx-auto">
                      ...
                      <div className="space-y-0">
                        {sortMenuItems(filterMenuItems(section.items)).map((item) => {
                          ...
                          return (
                            <div
                              key={item.id}
                              className="flex gap-4 p-4 border-b border-gray-100 last:border-none relative group hover:bg-neutral-50/50 transition-colors"
                              onClick={() => handleItemClick(item)}
                            >
                              {/* Left Side - Details matching Screenshot 2 */}
                              <div className="flex-1 min-w-0 space-y-1">
                                ...
                                {/* Pricing matching Screenshot 2 */}
                                <div className="flex items-center gap-1.5 py-0.5">
                                  <span className="font-black text-gray-900 dark:text-white text-sm">₹{Math.round(finalPrice)}</span>
                                  {originalPrice > finalPrice && (
                                    <span className="text-xs font-bold text-gray-300 line-through">₹{originalPrice}</span>
                                  )}
                                </div>
                              ...
                              <div className="relative w-36 h-36 flex-shrink-0">
                                <div className="w-full h-full rounded-2xl overflow-hidden shadow-sm border border-neutral-100">
                                  {item.image ? (
                                    <img
                                      src={item.image}
                                      alt={item.name}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className={`flex items-center gap-2 px-4 py-2.5 rounded-lg border-2 transition-all ${vegMode ? "w-full" : "flex-1"} ${filters.vegNonVeg === "veg"
              ...
              <>
                ...
                >
                  ...
                  <div className="px-4 py-4 space-y-2">
                    ...
                    // ...'Create new Collection' button removed...
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto px-4 py-4">
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-2 flex-1">
                        {selectedItem.foodType?.toLowerCase() === "veg" ? (
                          <div className="w-5 h-5 border-2 border-green-600 flex items-center justify-center rounded-sm flex-shrink-0">
                            <div className="w-2.5 h-2.5 bg-green-600 rounded-full"></div>
                          </div>
                        ) : selectedItem.foodType?.toLowerCase() === "egg" ? (
                          <div className="w-5 h-5 border-2 border-amber-500 flex items-center justify-center rounded-sm flex-shrink-0">
