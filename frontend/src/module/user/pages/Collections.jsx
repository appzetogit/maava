...
export default function Collections() {
  ...
  // In Mart collections
  const [inMartCollections, setInMartCollections] = useState([
    { id: "bookmarks", name: "Bookmarks", dishes: 0, restaurants: 0, isDefault: true }
  ])

  const currentCollections = activeTab === "delivery" ? deliveryCollections : inMartCollections
  const setCurrentCollections = activeTab === "delivery" ? setDeliveryCollections : setInMartCollections
    ...
    <div className="min-h-screen bg-white dark:bg-[#0a0a0a]">
      ...
      <div className="sticky top-0 bg-white dark:bg-[#1a1a1a] z-10 border-b dark:border-gray-800">
        <div className="flex">
          ...
          <button
            onClick={() => setActiveTab("inMart")}
            className={`flex-1 py-4 text-center font-semibold transition-colors relative ${activeTab === "inMart" ? "text-gray-900 dark:text-gray-100" : "text-gray-400 dark:text-gray-500"
              }`}
          >
            In Mart
            {activeTab === "inMart" && (
