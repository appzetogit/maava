...
export async function findNearestDeliveryBoys(restaurantLat, restaurantLng, restaurantId = null, priorityDistance = 5) {
  try {
    ...
    // Partners without valid GPS location get a very large distance (treated as far away)
    // but are still included so they receive socket notifications
    const LARGE_DISTANCE = 9999; // km - used for partners without location
    ...
    console.log(`✅ Found ${deliveryPartnersWithDistance.length} priority delivery partners (within ${priorityDistance}km or no location)`);
    return deliveryPartnersWithDistance.map(partner => ({
      deliveryPartnerId: partner._id.toString(),
      name: partner.name,
      phone: partner.phone,
      distance: partner.hasValidLocation ? partner.distance : null,
      location: partner.hasValidLocation ? {
