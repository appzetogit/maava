 ...
 * Processes and broadcasts location via WebSocket + Firebase Realtime DB
 */
...
import {
  ...
  saveOrderPolylineToFirebase,
  clearOrderFromFirebase
} from '../services/deliveryFirebaseService.js';
...
export const receiveLocationUpdate = asyncHandler(async (req, res) => {
  try {
    ...
    // ─── Write live location to Firebase Realtime DB (cost-free for readers) ──────
    // This allows the user tracking screen to listen via Firebase instead of polling MongoDB
    // No Maps API calls involved — just a simple database write
    updateDeliveryBoyLocation(
      deliveryBoyId,
      orderId,
      processedLocation.lat,
      processedLocation.lng,
      processedLocation.bearing,
      processedLocation.speed
    ).catch(err => console.warn('⚠️ Firebase location write failed (non-critical):', err.message));
    // ──────────────────────────────────────────────────────────────────────────────
...
export const initializeRoute = asyncHandler(async (req, res) => {
  try {
    ...
    // Cache route locally (in-memory)
    cacheRoutePolyline(orderId, route);

    // ─── Save polyline to Firebase (ONCE per order — all reads are free after this) ─
    const deliveryBoyId = req.deliveryBoy?.id || req.user?.id || 'unknown';
    const totalDistanceKm = route.totalDistance ? route.totalDistance / 1000 : 0;
    saveOrderPolylineToFirebase(
      orderId,
      deliveryBoyId,
      route.polyline,
      totalDistanceKm,
      route.duration || 0
    ).catch(err => console.warn('⚠️ Firebase polyline save failed (non-critical):', err.message));
    // ──────────────────────────────────────────────────────────────────────────────
