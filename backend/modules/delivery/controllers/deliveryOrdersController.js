...
export const acceptOrder = asyncHandler(async (req, res) => {
  try {
    ...
    // Get restaurant location (optional — missing location means we skip route calculation but still accept)
    let restaurantLat = null, restaurantLng = null;
    try {
      const restaurantData = order.restaurantId; // populated

      // Helper to extract lat/lng from a location object (handles both formats)
      const extractCoords = (loc) => {
        if (!loc) return null;
        // Try GeoJSON coordinates array first: [lng, lat]
        if (loc.coordinates && Array.isArray(loc.coordinates) && loc.coordinates.length >= 2) {
          const [lng, lat] = loc.coordinates;
          if (!(lng === 0 && lat === 0) && !isNaN(lat) && !isNaN(lng)) {
            return { lat, lng };
          }
        }
        // Fallback to explicit latitude/longitude fields
      ...
      // 1. Try from populated restaurantId.location
      if (restaurantData && restaurantData.location) {
        const coords = extractCoords(restaurantData.location);
        if (coords) {
      ...
      // 2. Try restaurantLocation embedded directly in order (set during notification)
      if (!restaurantLat || !restaurantLng) {
        const embedded = extractCoords(order.restaurantLocation);
        if (embedded) {
          restaurantLat = embedded.lat;
          restaurantLng = embedded.lng;
          console.log(`📍 Restaurant location from order.restaurantLocation: lat=${restaurantLat}, lng=${restaurantLng}`);
        }
      }

      // 3. Hibermart fallback
      if (!restaurantLat || !restaurantLng) {
        const isHibermartOrder =
          order.isHibermartOrder ||
          order.restaurantId === 'hibermart-id' ||
        ...
        if (isHibermartOrder) {
          const storeDoc = await HibermartStoreLocation.getOrCreate();
          const storeLoc = storeDoc?.location;
          const storeCoords = extractCoords(storeLoc);
          if (storeCoords) {
            restaurantLat = storeCoords.lat;
            restaurantLng = storeCoords.lng;
            console.log(`📍 Hibermart store location: lat=${restaurantLat}, lng=${restaurantLng}`);
          }
        }
      }

      // 4. Fetch restaurant directly from DB (covers cases where populate failed or location not in schema)
      if (!restaurantLat || !restaurantLng) {
        const restaurantId = restaurantData?._id || restaurantData || order.restaurantId;
        console.log(`⚠️ Restaurant location not in order data, fetching from DB (id: ${restaurantId})...`);
        if (restaurantId) {
          try {
            const restaurant = await Restaurant.findById(restaurantId).select('location').lean();
            const dbCoords = extractCoords(restaurant?.location);
            if (dbCoords) {
              restaurantLat = dbCoords.lat;
              restaurantLng = dbCoords.lng;
              console.log(`📍 Restaurant location from DB: lat=${restaurantLat}, lng=${restaurantLng}`);
            } else {
              console.warn(`⚠️ Restaurant ${restaurantId} found in DB but has no valid location coordinates`);
              console.warn(`⚠️ Restaurant location data:`, JSON.stringify(restaurant?.location));
            }
          } catch (dbErr) {
            console.warn(`⚠️ Could not fetch restaurant from DB: ${dbErr.message}`);
          }
        }
      }

      if (!restaurantLat || !restaurantLng) {
        console.warn(`⚠️ Restaurant location not found anywhere — will accept order without route calculation`);
      }
    } catch (locationError) {
      console.warn(`⚠️ Error getting restaurant location (non-fatal): ${locationError.message}`);
      // Don't return error — continue and skip route calculation
    }

    // Get delivery boy's current location (optional — missing location skips route calc but doesn't block acceptance)
    let deliveryLat = currentLat ? parseFloat(currentLat) : null;
    let deliveryLng = currentLng ? parseFloat(currentLng) : null;
    ...
    if (!deliveryLat || !deliveryLng) {
      ...
      try {
        ...
        if (deliveryPartner?.availability?.currentLocation?.coordinates) {
          const coords = deliveryPartner.availability.currentLocation.coordinates;
          if (coords.length >= 2 && !(coords[0] === 0 && coords[1] === 0)) {
            [deliveryLng, deliveryLat] = coords;
            console.log(`📍 Delivery location from profile: lat=${deliveryLat}, lng=${deliveryLng}`);
          }
        }
        if (!deliveryLat || !deliveryLng) {
          console.warn(`⚠️ Delivery partner location not available — will accept order without route calculation`);
        }
      } catch (deliveryLocationError) {
        console.warn(`⚠️ Error fetching delivery partner location (non-fatal): ${deliveryLocationError.message}`);
        // Don't block — continue without location
      }
    }

    // Calculate route if we have both locations; otherwise skip gracefully
    ...
    let routeData = null;
    const hasValidDeliveryCoords = deliveryLat && deliveryLng && !isNaN(deliveryLat) && !isNaN(deliveryLng);
    const hasValidRestaurantCoords = restaurantLat && restaurantLng && !isNaN(restaurantLat) && !isNaN(restaurantLng);

    if (hasValidDeliveryCoords && hasValidRestaurantCoords) {
      console.log(`✅ Valid coordinates — Delivery: (${deliveryLat}, ${deliveryLng}), Restaurant: (${restaurantLat}, ${restaurantLng})`);
      ...
      try {
        ...
        if (!routeData || !routeData.coordinates || !Array.isArray(routeData.coordinates) ||
          routeData.coordinates.length === 0 || isNaN(routeData.distance) || isNaN(routeData.duration)) {
          throw new Error('Invalid route data from calculateRoute');
        }
        console.log(`✅ Route calculated: ${routeData.distance.toFixed(2)} km, ${routeData.duration.toFixed(1)} mins`);
      } catch (routeError) {
        console.warn('⚠️ Route calculation failed, using haversine fallback:', routeError.message);
      ...
      // No location data — create a minimal placeholder route so order can still be accepted
      console.warn(`⚠️ Cannot calculate route — missing coordinates (delivery: ${hasValidDeliveryCoords}, restaurant: ${hasValidRestaurantCoords}). Accepting order without route.`);
      routeData = {
        coordinates: [],
        distance: 0,
        duration: 0,
    ...
    console.log(`✅ Route data ready (method: ${routeData.method})`);
